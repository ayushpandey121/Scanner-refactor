...
class ImageProcessor:
    ...
    def extract_all_grains(image, image_name, pixels_per_metric):
        ...
        for idx, (grain_rgb, bbox, contour) in enumerate(grains):
            ...
            exclude_grain = grain_area > threshold_area
            
            if exclude_grain:
                logger.info(f"Excluding grain {idx+1}: area {grain_area:.2f} > {threshold_area:.2f}")
        ...
        logger.info(f"Extracted {len(result)} grains")
        return result